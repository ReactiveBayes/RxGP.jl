# This file defines the Batch Sparse Gaussian Process (SGP) node for univariate case
# y_j = f(x_j), j=1..N_j, where y_j ∈ ℝ and x_j ∈ ℝᴰ
# out ∈ ℝ^{N_j} (stacked scalar outputs), in ∈ ℝ^{D·N_j} (stacked D-dimensional inputs)

export UniSGP_Batch

struct UniSGP_Batch end

@node UniSGP_Batch Stochastic [out, in, v, w, θ]

#---- Define average energy ----#
@average_energy UniSGP_Batch (q_out::IN_OUT, q_in::IN_OUT, q_v::MultivariateNormalDistributionsFamily, q_w::NOISE_w, q_θ::PointMass, meta::UniSGPMeta,) = begin
    θ = mean(q_θ)
    w_bar = mean(q_w)
    E_logw = mean(log, q_w)
    ...
    return 0.5*(w_bar*I_sum - N_j*E_logw + N_j*log(2π))
end
