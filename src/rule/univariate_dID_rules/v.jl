# rule for "v" edge (univariate dID case)
@rule UniSGP_dID(:v, Marginalisation) (q_out::IN_OUT, q_in::IN_OUT, q_Wg::NOISE_Wg, q_θ::PointMass, meta::UniSGPMeta) = begin
    θ = mean(q_θ)
    ...
    if q_in isa Distribution
        ...
        Ω3 = approximate_kernel_expectation(meta.method, (x) -> transpose(Kxu_fn(x, θ, meta.Xu))*Wg_bar*Kxu_fn(x, θ, meta.Xu), q_in)
        Ω4 = approximate_kernel_expectation(meta.method, (x) -> transpose(Lm_fn(x))*Wg_bar*Kxu_fn(x, θ, meta.Xu), q_in)
    else
        Ω1 = Kxu_fn(μ_in, θ, meta.Xu)
        Ω3 = transpose(Ω1) * Wg_bar * Ω1
        Ω4 = transpose(Lm_fn(μ_in)) * Wg_bar * Ω1
    end
