...
@testitem "helper_functions/approximate_kernel/Test predict_GP_values" setup=[setup_snippet] begin
    ...
    @test value_pred_means == [1.426392254114819, 1.5048343203596062, -213.0]
    @test value_pred_var == [12.295892216916247, 1.7208706079583371, 0.95]
    ...
    @test value_pred_means == [1.2984664489437887, 1.251521520847584, -208.6]
    @test value_pred_var == [2.8484427880442604, 1.0067644853938489, 0.95]
    ...
    @test value_pred_means == [0.5818501472216481, 4.582220642228733, -208.5]
    @test value_pred_var == [2.906792054169222, 0.9501651809610067, 0.95]
...
@testitem "helper_functions/approximate_kernel/Test predict_GP_gradients" setup=[setup_snippet] begin
    ...
    @test (grad_pred_means, grad_pred_cov) == ([[-0.673068277833309], [1.202928520977025], [1.0]], [[2.39163541314601;;], [3.402633523216112;;], [0.8616780045351473;;]])

    ctx = test_fixture(;D=2)
    grad_pred_means, grad_pred_cov = predict_GP_gradients(; m_in=[[1.3, 0.5], [2.5, -1.2], [-213, 4.4]], q_v=ctx.q_v, q_θ=ctx.q_θ, meta=ctx.meta)
    @test (grad_pred_means, grad_pred_cov) == ([[0.032305775592376906, 0.284284625145349], [0.8139161086436225, 0.7321200422666407], [1.0, 1.0]], [[2.025067843061719 1.0285131398444358; 1.0285131398444343 1.8068230232745943], [1.1671355815266986 -0.10958746306617301; -0.10958746306617298 0.8862106194320802], [0.95 0.0; 0.0 0.8616780045351473]])
...
@testitem "helper_functions/approximate_kernel/Test predict_GP_joints" setup=[setup_snippet] begin
    ...
    @test (joint_pred_means, joint_pred_grads) == ([1.426392254114819 -0.673068277833309; 1.5048343203596062 1.202928520977025; -213.0 1.0], [12.295892216916247 -9.425995231080496; 1.7208706079583371 -3.3011008425616613; 0.95 0.0;;; -9.425995231080496 2.39163541314601; -3.3011008425616613 3.402633523216112; 0.0 0.8616780045351473])
    
    ctx = test_fixture(;D=2)
    joint_pred_means, joint_pred_grads = predict_GP_joints(; m_in=[[1.3, 0.5], [2.5, -1.2], [-213, 4.4]], q_v=ctx.q_v, q_θ=ctx.q_θ, meta=ctx.meta)
    @test (joint_pred_means, joint_pred_grads) == ([1.2984664489437887 0.032305775592376906 0.284284625145349; 1.251521520847584 0.8139161086436225 0.7321200422666407; -208.6 1.0 1.0], [2.8484427880442604 -3.4628430482407735 -3.1414526503728752; 1.0067644853938489 -0.2319717579533537 0.09214016820423497; 0.95 0.0 0.0;;; -3.4628430482407735 2.025067843061719 1.0285131398444343; -0.2319717579533537 1.1671355815266986 -0.10958746306617298; 0.0 0.95 0.0;;; -3.1414526503728752 1.0285131398444358 1.8068230232745943; 0.09214016820423497 -0.10958746306617301 0.8862106194320802; 0.0 0.0 0.8616780045351473])
